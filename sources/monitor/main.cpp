...
int main() {
	...
	if (PID == 0) {
		...
		const char* argv[] = { "mapper", pid_string.c_str(), MONITOR_DUMPS_PATH.data(), NULL };
		if (execv(MAPPER_PATH.data(), const_cast<char**>(argv)) != 0) {
			std::cerr << "Can't run mapper: " << errno << ": " << strerror(errno) << std::endl;
			return 1;
		}
	}
	...
	if (w == -1) {
		std::cerr << "waitpid(...) error: " << errno << ": " << strerror(errno) << std::endl;
		return 1;
	}
